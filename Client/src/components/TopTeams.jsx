...
const TopTeams = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate the winValue to make sure it's a valid number
    if (!winValue || isNaN(winValue)) {
      alert('Please enter a valid number for minimum wins');
      return;
    }

    try {
      const response = await axios.get(`http://localhost:5000/api/football/top-teams/${winValue}`);
